...
class ListenBrainzService: Service {
    ...
    func buildURLs(for track: Track) -> (artistURL: URL, albumURL: URL, trackURL: URL) {
        ...
        if let releaseMbid = serviceInfo?.releaseMbid {
            ...
            albumURL = URL(string: "https://listenbrainz.org/search/?search_term=\(encodedArtist)%20\(encodedAlbum)&search_type=album")!
        }
        ...
        if let recordingMbid = serviceInfo?.id {
            ...
            trackURL = URL(string: "https://listenbrainz.org/search/?search_term=\(encodedArtist)%20\(encodedTrack)&search_type=track")!
        }
